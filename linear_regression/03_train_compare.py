...
def interpret_feature_weights(results: dict) -> None:
    ...
    ols = results["models"]["LinearRegression"]
    coef = pd.Series(ols.coef_, index=names).sort_values(key=np.abs, ascending=False)
    print("\nLinearRegression |coefficients| (top 10) — linear salary signal:")
    print(coef.head(10).to_string())

    tree = results["models"]["DecisionTreeRegressor"]
    tree_imp = pd.Series(tree.feature_importances_, index=names).sort_values(ascending=False)
    print("\nDecisionTree feature_importances_ (top 10):")
    print(tree_imp.head(10).to_string())
    ...
    print(
        "\nInterpretation: features with near-zero OLS coefficients AND low tree/RF "
