...
Used by the numbered Part-1 scripts (02–05), multivariate.ipynb, and
API/prediction.py /retrain. Numbered filenames like
`02_feature_engineering.py` are not valid import targets, so the real
logic lives here as importable functions.
"""
...
def train_compare_models(
    ...
    *,
    ...
    results["models"]["DecisionTreeRegressor"] = tree
    results["metrics"]["DecisionTreeRegressor"] = {
        "train": _metrics(y_train, tree.predict(X_train_t)),
        "test": _metrics(y_test, tree.predict(X_test_t)),
    }
